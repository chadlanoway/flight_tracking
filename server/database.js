...
const loadCSV2 = async () => {
    ...
    let csvStream = fastcsv
        ...
        .on("end", function () {
            ...
            const query = `
  INSERT INTO ${process.env.dbTable}
    (id,
     time_stamp,
     reg_number,
     altitude,
...
`;
            ...
            pool.connect((err, client, done) => {
                ...
                try {
                    csvData.forEach(rawRow => {
                        console.log("*forEach Loop: row: ", rawRow);
                        ...
                        const altitude = (altStr && altStr !== 'undefined') ? parseFloat(altStr) : null;
                        const direction = (dirStr && dirStr !== 'undefined') ? parseFloat(dirStr) : null;
                        const speed = (speedStr && speedStr !== 'undefined') ? parseFloat(speedStr) : null;
                        const latitude = (latStr && latStr !== 'undefined') ? parseFloat(latStr) : null;
                        const longitude = (lonStr && lonStr !== 'undefined') ? parseFloat(lonStr) : null;
                        ...
                        client.query(query, values, (err, res) => {
                            if (err) {
                                console.error("Error in forEach:", err.stack);
                            } else {
                    ...
                    console.error("Error in loadCSV2():", error.stack);
                }
                finally {
